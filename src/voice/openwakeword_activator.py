...
class OpenWakeWordActivator:
    def __init__(self, wake_word="hey_mycroft"):
        ...
        try:
            self.model = openwakeword.Model(wakeword_models=["hey_mycroft"])
            print(
                f"{Fore.GREEN}openWakeWord loaded with hey_mycroft model!{Style.RESET_ALL}"
            )
        except Exception as e:
    ...
    def start_listening(self):
        ...
        if not self.is_listening:
            ...
            print(
                f"{Fore.GREEN}openWakeWord activator started. Say 'hey mycroft' to activate.{Style.RESET_ALL}"
            )
            ...
            while self.is_listening:
                ...
                try:
                    ...
                    for phrase, score in predictions.items():
                        if (
                            score > 0.3 and phrase == "hey_mycroft"
                        ):  # Only accept hey_mycroft
                            current_time = time.time()

                            # Check if enough time has passed since last activation (3 seconds)
                            if current_time - self.last_activation_time > 3:
                                print(
                                    f"{Fore.GREEN}Wake word detected! ({phrase}: {score:.3f}){Style.RESET_ALL}"
                                )
        ...
        try:
            ...
            for i in range(5):
                ...
                try:
                    ...
                    print(
                        f"Audio sample {i + 1}/5 - Volume: {volume:.2f}, Max prediction: {max_score:.3f}"
                    )
                except Exception:
            ...
            print(
                f"{Fore.YELLOW}This might be a macOS audio permissions issue.{Style.RESET_ALL}"
            )
            print(
                f"{Fore.YELLOW}Try: System Preferences > Security & Privacy > Microphone{Style.RESET_ALL}"
            )
            return False
